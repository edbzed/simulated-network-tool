...
export function drawNetworkConnection(
  ...
  drawConnectionLine(ctx, source, target, isActive);
  drawDirectionArrow(ctx, source, target, isActive);
  if (isActive) {
    drawDataFlowParticle(ctx, source, target);
  }
}
